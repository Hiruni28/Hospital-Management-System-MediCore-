...
if (isset($_GET['id'])) {
    ...
    if ($stmt->execute()) {
        ...
        header('Location:' . SITEURL . 'admin-panel/manage-bookinPackage.php?status=success&action=confirm_booking');
        exit();
    } else {
        // Redirect to the manage-booking page with error message
        header('Location:'.SITEURL.'admin-panel/manage-bookinPackage.php?status=error&action=confirm_booking');
        exit();
    }
