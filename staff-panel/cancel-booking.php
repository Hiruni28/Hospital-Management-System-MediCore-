...
if (isset($_GET['id'])) {
    ...
    if ($stmt->execute()) {
        ...
        header('Location:' . SITEURL . 'staff-panel/manage-bookinProgram.php?status=success&action=cancel_booking');
        exit();
    } else {
        // If there's an error, redirect with an error message
        header('Location:'.SITEURL.'staff-panel/manage-bookinProgram.php?status=error&action=cancel_booking');
        exit();
    }
} else {
