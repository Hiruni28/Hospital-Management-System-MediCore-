...
if (isset($_GET['id'])) {
    ...
    $sql = "UPDATE tbl_membershipBookings SET status = 'Confirmed' WHERE id = ?";
    ...
    if ($stmt->execute()) {
        ...
        header('Location:' . SITEURL . 'staff-panel/manage-bookinPackage.php?status=success&action=confirm_booking');
        exit();
    } else {
        // Redirect to the manage-booking page with error message
        header('Location:'.SITEURL.'staff-panel/manage-bookinPackage.php?status=error&action=confirm_booking');
        exit();
    }
    ...
    header('Location: staff-panel/manage-bookinProgram.php');
}
